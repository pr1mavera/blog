...
&emsp;&emsp;通过RxJS构建的流式应用，用一种响应式的方式构建前端复杂数据层，给我们带来了不一样的编程思路。今天记录一下对于其响应式原理的理解，如有理解错误的地方还望指出，非常感谢。

## 整体认识
&emsp;&emsp;RxJS是以Observable可观测数据管道为统一的IO monad，构建的异步函数式库，~~懂了吧~~，蒙了吧。蒙了没关系，网上很多说法开始我也是真的看不明白。  
...
&emsp;&emsp;通过RxJS构建的流式应用，能够构建响应式的数据流 `$a`、`$b`、`$c` ，形成 `$x = $a + $b + $c` 的数据管道，当其中任意一条数据流发生变化，便会顺着 `$a + $b + $c` 的数据管道流入 `$x` 触发更新操作，并且未发生变化的数据流是提供当前缓存的最新值做数据组合，实现了数据的缓存处理。  
&emsp;&emsp;这种形拉实推的数据通信方式，使得RxJS能够以一种流动的数据的方式，构建响应式应用。并且通过这种方式构建的应用，业务逻辑清晰可见、拓展性良好，是构建前端复杂数据逻辑层的利器。
...
## 编程范式
&emsp;&emsp;RxJS是函数式编程与响应式编程的结合，如何理解呢？
...
&emsp;&emsp;RxJS的所有操作符几乎都是纯函数，不产生副作用。  
...
### 关于响应式
&emsp;&emsp;关于响应式编程我的理解是，关键在于构建数据之间的关联关系，通过一处数据的变化引起和它关联的其他数据的更新，使得整个应用程序从顶层root层进行整体的数据流分发，流向下游的关联数据分支。这样构建应用程序的好处是数据的绑定关系是关注分离的，并且是响应式的，每条数据流的中间状态可以得到复用，从一定程度上实现了代码的复用性。  
&emsp;&emsp;RxJS将各种数据来源都通过Observable包装起来，在这个容器上实现各种各样的操作符，方便在统一的时间层面上分发控制管理这些数据流。
...
现在我们来想一个问题：为什么说RxJS是异步函数式库呢？函数式编程中实现异步的关键是惰性求值。惰性求值又是什么呢？惰性求值指的是在将表达式赋值给变量（或称作绑定）时并不计算表达式的值，而在变量第一次被使用时才进行计算。
...
```
这里仅将初始化Observable时传入的_subscribe保存在实例的属性里，当我们调用subscribe添加订阅的时候才真正开始消费生产者发送出来的数据。接下来我们调用一下刚写的Observable就明白了：
```js
